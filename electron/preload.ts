...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  commitAndPush: (path: string, message: string) =>
    ipcRenderer.invoke("git:commit-push", path, message),
  ...
  createPR: (fullName: string, title: string, body: string, head: string, base: string) =>
    ipcRenderer.invoke("github:create-pr", fullName, title, body, head, base),
  ...
  addAccount: (token: string) => ipcRenderer.invoke("github:add-account", token),
  getAccounts: () => ipcRenderer.invoke("github:get-accounts"),
  switchAccount: (username: string) => ipcRenderer.invoke("github:switch-account", username),
  deleteAccount: (username: string) => ipcRenderer.invoke("github:delete-account", username),
  getProfileStats: () => ipcRenderer.invoke("github:get-profile-stats"),
});
